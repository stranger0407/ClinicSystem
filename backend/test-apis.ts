...
async function runTests() {
  ...
  try {
    ...
    const doctorId = doctors[0]?.id; // User id or DoctorProfile id
    if (!doctorId) throw new Error('No doctor profile found in list');
    ...
    const docProfileId = doctors[0]?.doctorProfile?.id || doctorId;
    ...
    const medSearchRes = await fetch(`${BASE_URL}/encounter/medicine/search?q=Paracetamol`, { headers });
    const meds = await medSearchRes.json() as any;
    const medicineId = meds[0]?.id;
    if (!medicineId) throw new Error('Paracetamol medicine not found in master catalog');

    // Log in as Doctor to get Doctor JWT
    ...
    const docAuth = await docLoginRes.json() as any;
    ...
    if (!encounter.id) {
      console.error('Encounter creation failed. Response:', encounter);

        ...
        <div className="space-y-7">
          ...
          <div className="p-3.5 bg-slate-800/50 rounded-xl flex items-center space-x-3 border border-slate-700/30">
            ...
            <div className="overflow-hidden">
              ...
              <span className="text-[8px] text-slate-450 uppercase tracking-widest font-bold block mt-0.5">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-5">
                ...
                <div className="pt-2">
                  <h3 className="text-[10px] text-slate-450 font-black uppercase block border-b border-slate-100 pb-1.5">Registered Profile Info</h3>
              ...
              {doctors.map((doc) => (
                <div key={doc.id} className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex flex-col justify-between space-y-4">
                  ...
                  <div className="bg-slate-50 border border-slate-100 rounded-xl p-3 text-xs text-slate-700 grid grid-cols-2 gap-2.5">
                    ...
                    <div>
                      <span className="text-[9px] text-slate-450 block font-bold uppercase">Session Duration</span>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm max-w-xl">
              ...
              <form onSubmit={handleBookAppointment} className="space-y-4 text-slate-900">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="text-[10px] text-slate-500 font-bold uppercase">Date of Visit</label>
                    <input
                      type="date"
                      value={bookingDate}
                      required
                  ...
                  {bookingType === 'SLOT' && (
                    <div className="space-y-1">
                      <label className="text-[10px] text-slate-500 font-bold uppercase">Preferred Time Slot</label>
                      <input
                        type="time"
                        value={bookingTime}
                  ...
                  <div key={enc.id} className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                      ...
                      <div className="bg-slate-50 border border-slate-100 rounded-xl p-4 space-y-3">
                        <h4 className="text-[9px] text-slate-450 font-black uppercase tracking-wider flex items-center">
                          <Heart className="w-3.5 h-3.5 text-red-500 mr-1.5 animate-pulse" />
                          Vitals Check
                        </h4>
                        <div className="grid grid-cols-2 gap-2.5 text-xs text-slate-750">
                          <div>
                      ...
                      <div className="md:col-span-2 space-y-3">
                        <div>
                          <span className="text-[9px] text-slate-450 font-black uppercase block tracking-wider">Symptoms / Complaints</span>
                          <p className="text-xs text-slate-800 font-medium mt-0.5">{enc.complaint}</p>
                        </div>
                        <div>
                          <span className="text-[9px] text-slate-450 font-black uppercase block tracking-wider">Diagnosis</span>
                          <p className="text-xs text-slate-900 font-bold mt-0.5">{enc.diagnosis}</p>
                        </div>
                        {enc.clinicalNotes && (
                          <div>
                            <span className="text-[9px] text-slate-450 font-black uppercase block tracking-wider">Doctor Comments & Advice</span>
                            <p className="text-xs text-slate-650 italic mt-0.5">{enc.clinicalNotes}</p>
                          </div>
                        )}
                      </div>
                    </div>
                          ...
                          {enc.prescriptions.flatMap((p: any) => p.items).map((item: any) => (
                            <div key={item.id} className="p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 text-xs">
                              <div>
                                ...
                                <span className="text-slate-450 ml-1.5">({item.medicine.strength})</span>
                              </div>
          ...
          >
            ...
            <div className="flex justify-between items-start border-b border-slate-100 pb-4">
              <div>
                ...
                <p className="text-[10px] text-slate-450 mt-0.5">{(clinic as any)?.address || 'Clinic Subdomain: ' + clinic?.subdomain}</p>
              </div>
              <div className="text-right">
                <span className="font-bold text-slate-800 block text-sm">{selectedInvoice.invoiceNumber}</span>
                <span className="text-[10px] text-slate-450 block mt-0.5">Date: {new Date(selectedInvoice.createdAt).toLocaleDateString()}</span>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <span className="text-[9px] text-slate-450 font-black uppercase tracking-wider block">Bill Items</span>
            ...
            <div>
              <span className="text-[9px] text-slate-450 font-black uppercase tracking-wider block mb-1.5">Payments Ledger</span>
              <div className="space-y-2">
                {selectedInvoice.payments?.length > 0 ? (
                  selectedInvoice.payments.map((p: any) => (
                    <div key={p.id} className="bg-slate-50/70 border border-slate-100 rounded-lg p-2.5 flex items-center justify-between text-xs">
                      <div>
                        <span className="text-[10px] text-slate-450 font-bold uppercase block">Method: {p.method}</span>
